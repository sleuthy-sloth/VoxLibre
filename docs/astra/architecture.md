...
Practice uses a pure authored-answer evaluator. It never contacts a translation/model endpoint. Mode-specific append-only practice events feed a deterministic mastery projection and SM-2 schedules. Local events are stored durably in IndexedDB before reporting success and identified by mutation UUID. Guest and account stores are separate; explicit account selection enables authenticated synchronization with the additive FoundationPracticeEvent table. Existing account-backed travel course progress remains separate. Export/import permits recovery and merges events by ID. Account synchronization exchanges the same immutable events; completion, schedules and concept evidence are derived locally after merging.
